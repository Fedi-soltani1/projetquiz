var FormElements = function() {"use strict";
	...
	var touchSpinHandler = function() {
		$("input[name='formation[score]']").TouchSpin({
			min: 0,
			max: 100,
			step: 0.1,
			decimals: 2,
		...
		$("input[name='formation[timer]']").TouchSpin({
			min: 0,
			max: 100,
			step: 0.1,
			decimals: 2,
	...
	var timePickerHandler = function() {
		...
		$('#presentiel_session_time').timepicker();
	};

 /*   var timePickerHandler1 = function() {
        $('#eventekbundle_session_startTime').timepicker({
            showMeridian:false,
            minuteStep:5
        });
