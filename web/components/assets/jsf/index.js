...
var Index = function() {
	...
	var chart3Handler = function() {
		...
		$.get('ajax/api.php', function(response) {
			var ctx = $("#chart3").get(0).getContext("2d");
			var chart3 = new Chart(ctx).Doughnut(response, options);
			var legend = chart3.generateLegend();
			$('#chart3Legend').append(legend); }, 'json');
...
for (var i=0; i< $('.accordion .panel-white' ).length; i++) {

	var data = [{
		value: $('.chart'+i+'Legend').attr('data-ech'+i),
		color: '#F7464A',
		highlight: '#FF5A5E',
		label: 'Échec'
	}, {
		value: $('.chart'+i+'Legend').attr('data-suc'+i),
		color: '#46BFBD',
		highlight: '#5AD3D1',
		label: 'Réussite'
	}];
	...
	// This will get the first returned node in the jQuery collection.
	var chart0 = new Chart(ctx).Doughnut(data,{segmentStrokeWidth:0});
	//generate the legend
	var legend = chart0.generateLegend();

	$('.chart'+i+'Legend').append(legend);
	if(i > 0){
